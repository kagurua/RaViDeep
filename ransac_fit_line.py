        ...
        for _ in range(self.k):
            ...
            if inlier_ids.size > self.d:
                ...
                if this_error < self.best_error:
                    ...
                    self.best_fit_inner_num = inlier_ids.size
...
class Weighted_LinearRegressor_0:
    def __init__(self):
        self.params = None
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        W = np.diag(X[:, -1])
        X = X[:, :-1]
        self.params = np.linalg.pinv(X.T @ W @ X) @ X.T @ W @ y
        return self
...
if __name__ == "__main__":
    regressor = RANSAC(model=LinearRegressor(), loss=square_error_loss, metric=mean_square_error, t=1)
    # regressor = RANSAC(model=LinearRegressor_0(), loss=square_error_loss, metric=mean_square_error, t=1)
